...
public class Board
{
    ...
    //
    int normalItemsNumber;
    Dictionary<NormalItem.eNormalType, int> stocks;
    public Dictionary<NormalItem.eNormalType, List<Cell>> itemCells;
    //
    public Board(Transform transform, GameSettings gameSettings)
    {
        ...
        //
        normalItemsNumber = 7;
        stocks = new Dictionary<NormalItem.eNormalType, int>();
        for (int i = 0; i < normalItemsNumber; i++)
        {
            stocks[(NormalItem.eNormalType) i] = 3;
        }
        
        int randomItem = UnityEngine.Random.Range(0, normalItemsNumber);
        stocks[(NormalItem.eNormalType)randomItem] += 3;

        itemCells = new Dictionary<NormalItem.eNormalType, List<Cell>>();
        for (int i =0; i< 7; i++)
        {
            itemCells.Add((NormalItem.eNormalType)i, new List<Cell>());
        }
        //
        CreateBoard();
    }
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                while (true)
                {
                    ...
                    if (stocks[item.ItemType] > 0)
                    {
                        itemCells[item.ItemType].Add(cell);
                        stocks[item.ItemType]--;
                        item.lastPosition = cell;
                        break;
                    }
                }
                //
